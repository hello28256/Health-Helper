// StepsProvider 单元测试 —— 验证：
// 1. 列表成功 → 返回 List<DailyStep>
// 2. 网络失败 → 抛 ApiError
// 3. upsertToday 成功 → 列表更新（同日期替换）
...
void main() {
  ...
  test('list 成功 → 返回 List<DailyStep>', () async {
    final container = makeContainer();
    final list = await container.read(stepsProvider.future);
    expect(list, isA<List<DailyStep>>());
  ...
  test('list 网络失败 → 抛 ApiError', () async {
    ...
    final container = makeContainer();
    await expectLater(
      container.read(stepsProvider.future),
      throwsA(isA<ApiError>()),
    );
  });

  test('upsertToday 成功 → 同 date 替换', () async {
    // 初始列表已有 date=2026-08-11
    adapter = MockAdapter((req) async {
      if (req.method == 'POST') {
        return _ok(
          '{"userId":"u-1","date":"2026-08-11","steps":12000,"source":"manual","updatedAt":"2026-08-11T10:00:00Z"}',
        );
      }
      return _ok(
        '[{"userId":"u-1","date":"2026-08-11","steps":5000,"source":"manual","updatedAt":"2026-08-11T08:00:00Z"}]',
      );
    });
    final container = makeContainer();
    await container.read(stepsProvider.future);

    await container.read(stepsProvider.notifier).upsertToday(steps: 12000);
    final list = container.read(stepsProvider).value;
    expect(list, isNotNull);
    expect(list!.length, 1);
    expect(list.first.steps, 12000);
  });
}